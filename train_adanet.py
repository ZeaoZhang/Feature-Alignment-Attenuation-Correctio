...
def train(args, run):
    ...
    disc = adanet3D.Discriminator(nums=args.d_nums, in_channels=8192).cuda()
    # disc = adanet3D.Discriminator(nums=args.d_nums, in_channels=32768).cuda()
    ...
    for iter_num in pbar:
        for _ in range(len(train_iter)):
            ...
            # discriminator
            if args.use_disc and iter_num % args.n_critic == 0:
                with autocast():
                    outputs = model((ncs, ctas))
                    if args.use_wgan:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_workers', type=int, default=8, help="num works")
    ...
    parser.add_argument('--d_nums', type=int, default=4, help="d_nums")
    ...
    parser.add_argument('--disc_lambda', type=float, default=2.0, help="disc_lambda")
    ...
    parser.add_argument('--metric_lambda', type=float, default=0.5, help="metric_lambda")
